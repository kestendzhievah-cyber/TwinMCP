...
As a library owner, you can claim your library on TwinMCP to unlock advanced configuration options through a web-based admin panel. This gives you full control over how your documentation is parsed and presented to developers.
...
<Steps>
  <Step title="Navigate to the Admin Page">
    ...
    **Via Direct URL:**
    ...
    ```
    https://twinmcp.com/{owner}/{repo}/admin
    ```

    For example: `https://twinmcp.com/vercel/next.js/admin`
  ...
  <Step title="Get Your Public Key">
    In the claiming modal, you'll see a generated `twinmcp.json` configuration with your unique public key:
    ...
    ```json
    {
      "url": "https://twinmcp.com/vercel/next.js",
  ...
  <Step title="Host the twinmcp.json File">
    <Tabs>
      <Tab title="Git Repository">
        Create a `twinmcp.json` file in the **root** of your repository with the copied content:
        ...
        <Warning>
          The `url` must exactly match your library's URL on TwinMCP, and the `public_key` must match the key shown in the modal.
        </Warning>

        Commit and push the file to your repository's default branch.
      </Tab>
      <Tab title="Website / llms.txt">
        Host the `twinmcp.json` file anywhere under your library's base URL so it is publicly reachable. For example:

        ```
        https://docs.example.com/mylib/twinmcp.json
        ```
        ...
        ```json
        {
          "url": "https://twinmcp.com/websites/mylib",
          "public_key": "pk_abc123xyz..."
        }
        ```
        ...
        Once the file is live, paste the full URL to your hosted `twinmcp.json` into the input field shown in the modal.
      </Tab>
    </Tabs>
  </Step>

  <Step title="Verify Ownership">
    Click "Claim Library". TwinMCP will fetch your `twinmcp.json`, verify the URL and public key, and grant you access to the admin panel.
  </Step>
</Steps>
...
| ----------------- | --------------------------------------------------------------------------------------------------------- | ------------------ |
| **Project Title** | Display name for your library in TwinMCP. Used when the LLM cannot generate a name with high confidence. | Max 100 characters |
...
| **Disallow Indexing** | Opt-out from TwinMCP. When enabled, documentation content is removed and the library becomes inaccessible. | Boolean            |

## Managing Versions

The Versions tab lets you configure previous versions of your library that should be available in TwinMCP.
...
The Metrics tab provides insights into how developers are using your library through TwinMCP.
...
| Metric                 | Description                                                           |
| ---------------------- | --------------------------------------------------------------------- |
| **Page Views**         | Number of times your library page was viewed on TwinMCP              |
...
<Steps>
  <Step title="Go to the Admin Page">
    Navigate to your library's admin page at `https://twinmcp.com/admin/{owner}/{repo}`.
  </Step>
...
### "twinmcp.json not found"

Ensure the file is:

- Named exactly `twinmcp.json` (lowercase)
- Located in the repository root, not a subdirectory
- Committed and pushed to the default branch

### "URL mismatch"

The `url` field in your `twinmcp.json` must exactly match:

```
https://twinmcp.com/{owner}/{repo}
```
