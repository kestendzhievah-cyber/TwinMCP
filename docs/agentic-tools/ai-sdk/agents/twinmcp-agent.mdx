---
title: "TwinMCPAgent"
sidebarTitle: "TwinMCPAgent"
description: "Pre-built AI agent for documentation lookup workflows"
---

The `TwinMCPAgent` class is a pre-configured AI agent that handles the complete documentation lookup workflow automatically. It combines both `resolveLibraryId` and `queryDocs` tools with an optimized system prompt.
...
```typescript
new TwinMCPAgent(config?: TwinMCPAgentConfig)
```

### Parameters

<ParamField path="config" type="TwinMCPAgentConfig" optional>
...
`TwinMCPAgent` extends the AI SDK `Agent` class and provides `generate()` and `stream()` methods.
...
2. **Resolve library** - Calls `resolveLibraryId` to find the TwinMCP library ID
...
```typescript
import { TwinMCPAgent, AGENT_PROMPT } from "@upstash/twinmcp-tools-ai-sdk";
import { openai } from "@ai-sdk/openai";
...
| Feature       | TwinMCPAgent        | Individual Tools     |
| ------------- | -------------------- | -------------------- |
