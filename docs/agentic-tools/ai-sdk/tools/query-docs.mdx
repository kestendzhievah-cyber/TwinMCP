...
The `queryDocs` tool fetches documentation for a library using its TwinMCP-compatible library ID and a query. This tool is typically called after `resolveLibraryId` has identified the correct library.
...
```typescript
queryDocs(config?: TwinMCPToolsConfig)
```

### Parameters

<ParamField path="config" type="TwinMCPToolsConfig" optional>
...
2. Fetches documentation from TwinMCP's API
...
<ParamField path="libraryId" type="string" required>
  TwinMCP-compatible library ID (e.g., `/reactjs/react.dev`, `/vercel/next.js`)
</ParamField>
...
```
No documentation found for library "/invalid/library". This might have happened because you used an invalid TwinMCP-compatible library ID. Use 'resolveLibraryId' to get a valid ID.
```
...
```typescript
import { queryDocs } from "@upstash/twinmcp-tools-ai-sdk";

const tool = queryDocs({
  apiKey: process.env.TWINMCP_API_KEY,
});
```
...
- [TwinMCPAgent](/agentic-tools/ai-sdk/agents/twinmcp-agent) - Pre-built agent that handles the full workflow
