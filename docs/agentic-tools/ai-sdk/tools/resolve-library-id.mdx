...
description: "Search for libraries and resolve them to TwinMCP-compatible IDs"
---

The `resolveLibraryId` tool searches TwinMCP's library database and returns matching results with their TwinMCP-compatible library IDs. This is typically the first step in a documentation lookup workflow.

## Usage

```typescript
import { resolveLibraryId } from "@upstash/twinmcp-tools-ai-sdk";
...
```typescript
resolveLibraryId(config?: TwinMCPToolsConfig)
```
...
2. Searches TwinMCP's database for matching libraries
...
- TwinMCP-compatible library ID: /reactjs/react.dev
...
----------
- Title: React Native
- TwinMCP-compatible library ID: /facebook/react-native
