...
This guide helps you resolve common issues when setting up or using TwinMCP MCP.

## Quick Checklist

- Confirm Node.js v18+ is installed (`node --version`)
- Update to the latest TwinMCP MCP package (`@upstash/twinmcp-mcp@latest`)
- Verify connectivity with `curl https://mcp.twinmcp.com/ping`
...
<Tip>
  **Skip Node.js issues entirely**: Use the remote server connection instead of local npx. Most MCP clients support connecting to `https://mcp.twinmcp.com/mcp` directly, which requires no local Node.js installation. See [All MCP Clients](/resources/all-clients) for configuration examples.
</Tip>
...
```json
{
  "mcpServers": {
    "twinmcp": {
      "command": "npx",
      "args": ["-y", "@upstash/twinmcp-mcp@latest", "--api-key", "YOUR_API_KEY"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "twinmcp": {
      ...
      "args": [
        "C:\\Users\\yourname\\AppData\\Roaming\\npm\\node_modules\\@upstash\\twinmcp-mcp\\dist\\index.js",
        "--transport",
        "stdio",
        "--api-key",
        "YOUR_API_KEY"
...
```json
{
  "mcpServers": {
    "twinmcp": {
      ...
      "args": [
        "/Users/yourname/.nvm/versions/node/v22.14.0/lib/node_modules/@upstash/twinmcp-mcp/dist/index.js",
        "--transport",
        "stdio",
        "--api-key",
        "YOUR_API_KEY"
...
- **Get an API key**: Sign up at [twinmcp.com/dashboard](https://twinmcp.com/dashboard) for higher rate limits
...
3. **Check if the library exists** at [twinmcp.com](https://twinmcp.com)
...
- Check logs with `claude mcp logs twinmcp`
- Try removing and re-adding the server

## Configure HTTPS Proxy

If you're behind a corporate proxy, configure TwinMCP to route through it.
...
<Tip>
  After updating proxy settings, run `curl https://mcp.twinmcp.com/ping` to confirm outbound
...
This opens an interactive inspector to test TwinMCP tools.
...
```bash
curl https://mcp.twinmcp.com/ping
```
...
1. **Check GitHub Issues**: Search for similar problems at [github.com/upstash/twinmcp/issues](https://github.com/upstash/twinmcp/issues)
...
- [Installation Instructions](https://github.com/upstash/twinmcp#installation)
