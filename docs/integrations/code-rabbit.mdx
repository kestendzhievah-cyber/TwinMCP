...
<Steps>
  ...
  <Step title="Add TwinMCP">
    Click the **Add** button next to **TwinMCP** MCP server.

    Add the following header for higher rate limits:

    | Header | Value |
    |--------|-------|
    | `TWINMCP_API_KEY` | Your API key from the [TwinMCP dashboard](https://twinmcp.com/dashboard) |
  ...
  <Step title="Enable for Public Repos">
    To use TwinMCP with public repositories, go to **Organization Settings** → **Configuration** → **Knowledge Base**. In the MCP section, select **Enabled** instead of **Auto** and click **Apply Changes**.
...
In the **Usage Guidance** field, you can instruct CodeRabbit on how to use TwinMCP during reviews. For example:

```
Use TwinMCP to look up documentation for any libraries used in the code being reviewed.
Verify that API usage matches the latest documentation and flag any deprecated patterns.
```

## How It Works

Once connected, CodeRabbit will query TwinMCP for relevant library documentation when reviewing pull requests. This helps CodeRabbit:
