---
title: API Guide
description: Authentication, rate limits, best practices, and integration guides for the TwinMCP API
---
...
```bash
Authorization: Bearer TWINMCP_API_KEY
```

Get your API key at [twinmcp.com/dashboard](https://twinmcp.com/dashboard). Learn more about [creating and managing API keys](/howto/api-keys).
...
headers = {"Authorization": "Bearer TWINMCP_API_KEY"}

# Step 1: Search for the library
search_response = requests.get(
    "https://twinmcp.com/api/v2/libs/search",
    headers=headers,
...
context_response = requests.get(
    "https://twinmcp.com/api/v2/context",
    headers=headers,
...
- View current usage and reset windows in the [dashboard](https://twinmcp.com/dashboard).
...
curl "https://twinmcp.com/api/v2/context?libraryId=/vercel/next.js&query=How%20to%20implement%20authentication%20with%20middleware" \
  -H "Authorization: Bearer TWINMCP_API_KEY"

# Less optimal - vague query
curl "https://twinmcp.com/api/v2/context?libraryId=/vercel/next.js&query=auth" \
  -H "Authorization: Bearer TWINMCP_API_KEY"
```
...
```bash
curl "https://twinmcp.com/api/v2/context?libraryId=/vercel/next.js/v15.1.8&query=app%20router" \
  -H "Authorization: Bearer TWINMCP_API_KEY"
...
The TwinMCP API uses standard HTTP status codes:
