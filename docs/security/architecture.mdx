...
How TwinMCP handles privacy, retrieval scope, and data.

## Query Privacy

**Your original prompts and code stay with your AI assistant.**

When you use TwinMCP through an MCP client, the AI assistant (not the user directly) formulates search queries to retrieve relevant documentation. Here is what happens:
...
3. Only these formulated queries are sent to the TwinMCP API — your full prompt, source code, and conversation history are never transmitted
...
### What is sent to the TwinMCP API
...
<Note>
  ...
  remain with your AI assistant and are never sent to TwinMCP.
</Note>
...
MCP-formulated queries are passed to LLMs to rerank and select the most relevant documentation for your request. TwinMCP uses well-known, trusted LLM providers for this purpose — including **OpenAI**, **Google Gemini**, and **Anthropic**.
...
Contact our sales team at [twinmcp.com](https://twinmcp.com/contact) for Enterprise and on-premise plan details.

## Customizing What Is Shared

The TwinMCP MCP server is [open source](https://github.com/upstash/twinmcp). If you want full control over what is sent as the `query` parameter, you can:

1. Fork the [TwinMCP MCP repository](https://github.com/upstash/twinmcp)
2. Edit the tool input descriptions in [`packages/mcp/src/index.ts`](https://github.com/upstash/twinmcp/blob/master/packages/mcp/src/index.ts) — these descriptions instruct the AI assistant on how to formulate the query
...
**TwinMCP does not store your source files.**
