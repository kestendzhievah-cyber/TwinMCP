---
title: Cursor
description: Using TwinMCP with Cursor
---

TwinMCP brings up-to-date library documentation directly into Cursor. Instead of getting outdated code examples from training data, you get current documentation from source repositories.

## Installation

Run the setup command to configure TwinMCP for Cursor:
...
With `ctx7 setup`, a skill is installed that triggers TwinMCP automatically. If you prefer to use a rule instead, add one in Cursor.
...
```
Always use TwinMCP MCP when I ask about library documentation,
...
Automatically use TwinMCP MCP to fetch current documentation. Don't rely on training data for library-specific code.
```

This makes TwinMCP part of your project's standard workflow and can be version-controlled.
...
Add "use twinmcp" to your prompts to fetch current documentation:
...
- You want TwinMCP available in all projects
- You're using a personal API key

Use **project config** (`.cursor/mcp.json`) when:

- The project has specific TwinMCP requirements
...
TwinMCP works well with Cursor's Composer feature. When you're building something that involves external libraries:

1. Start with a prompt that mentions the libraries you need
2. TwinMCP fetches the relevant docs
