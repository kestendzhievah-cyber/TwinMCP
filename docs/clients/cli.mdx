---
title: CLI
description: The ctx7 CLI — fetch library documentation, manage skills, and configure TwinMCP MCP from your terminal
---

The `ctx7` CLI is the command-line interface for TwinMCP. It does three things:
...
- **Manage AI coding skills** — install, search, generate, and remove skills from the TwinMCP registry
- **Configure TwinMCP MCP** — set up the MCP server for Claude Code, Cursor, or OpenCode with a single command

The CLI is useful both as a standalone tool (fetching docs while you code) and as a setup utility (wiring up TwinMCP for your AI coding agent).
...
Fetching docs is a two-step process: first resolve the library name to get its TwinMCP ID, then use that ID to query documentation.

### Step 1 — ctx7 library

Searches the TwinMCP index by name and returns matching libraries. Pass a `query` describing what you're trying to do — this ranks results by relevance and helps when a library name is ambiguous or shared across multiple packages.
...
Configure TwinMCP for your AI coding agent. On first run, prompts you to choose between two modes:

- **MCP server** — registers the TwinMCP MCP server in your agent's config so it can call `resolve-library-id` and `get-library-docs` tools natively
...
| Agent rules directory | Rule file — instructs the agent to use TwinMCP for library docs |
| Agent skills directory | `twinmcp-mcp` skill |
...
Manage AI coding skills from the TwinMCP registry. See [Skills](/skills) for a full explanation of what skills are, the registry, trust scores, and skill file structure.
...
<CardGroup cols={2}>
  <Card title="Browse Skills" icon="magnifying-glass" href="https://twinmcp.com/skills">
