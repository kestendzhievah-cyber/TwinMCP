...
TwinMCP allows you to add your favorite libraries and frameworks so developers always receive current, trustworthy documentation inside their coding environment.


## Claiming Your Library

If you're the library owner, you can claim your library on TwinMCP to unlock a web-based admin panel for managing configuration. This allows you to:
...
- **Library authors**: Claim ownership via `twinmcp.json` and manage settings through the admin panel, or add configuration directly to your repository
...
**[Submit a Library ->](https://twinmcp.com/add-library?tab=github)**

1. Paste the GitHub repository URL.
2. (Optional) Adjust folders and exclusions.
3. Submit and let TwinMCP parse and index the documentation.
...
## Advanced Configuration with `twinmcp.json`

For more control over how TwinMCP parses and presents your library, you can add a `twinmcp.json` file to the root of your repository. This file works similarly to `robots.txt` and tells TwinMCP how to handle your project.

### Configuration Fields

Here's an example `twinmcp.json` file with all available options:

```json
{
  "$schema": "https://twinmcp.com/schema/twinmcp.json",
...
- **`projectTitle`** (string): Suggested display name for your project in TwinMCP. Only used when the LLM cannot generate a name with high confidence.

- **`description`** (string): Suggested description for your project in TwinMCP. Only used when the LLM cannot generate a description with high confidence.

- **`branch`** (string): The name of the git branch to parse. If not provided, the default branch will be used.

- **`folders`** (array): Specific folder paths to include when parsing. If empty, TwinMCP scans the entire repository. Root-level markdown files are always included.
...
- **`previousVersions`** (array): Information about previous versions of your library that should also be available in TwinMCP.
...
If you don't specify `excludeFiles` or `excludeFolders` in your `twinmcp.json` file, TwinMCP uses these default patterns:
...
## Need Help?
If you encounter issues or need assistance adding your project, please [open an issue](https://github.com/upstash/twinmcp/issues/new/choose) or reach out to our community.
