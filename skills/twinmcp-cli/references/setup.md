...
One-time command to configure TwinMCP for your AI coding agent. Prompts for mode on first run:
- **MCP server** — registers the TwinMCP MCP server so the agent can call tools natively
...
- A TwinMCP rule file instructing the agent to use TwinMCP for library docs
- A `twinmcp-mcp` skill in the agent's skills directory
