...
export class LibraryResolutionService {
  ...
  async resolveLibrary(input: ResolveLibraryIdInput): Promise<ResolveLibraryIdOutput> {
    ...
    try {
      // Vérifier le cache
      const cacheKey = `library_resolution:${Buffer.from(
        JSON.stringify({
      ...
      const suggestions =
        finalResults.length === 0 ? await this.generateSuggestions(normalizedQuery) : undefined;
      ...
      throw new Error(
        `Library resolution failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async generateSuggestions(normalizedQuery: string): Promise<string[]> {
    ...
    suggestions.push(...similarLibraries.map((lib: (typeof similarLibraries)[number]) => lib.name));
