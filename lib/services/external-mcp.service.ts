...
export class ExternalMcpService {
  ...
  async update(
    id: string,
    ownerId: string,
    input: Partial<CreateExternalMcpServerInput>
  ): Promise<ExternalMcpServerDTO> {
    ...
    if (input.secret !== undefined)
      data.encryptedSecret = input.secret ? encrypt(input.secret) : null;
  ...
  async healthCheck(
    id: string,
    ownerId: string
  ): Promise<{ status: string; latencyMs: number; tools?: any[]; error?: string }> {
  ...
  async getUsage(userId: string, serverId?: string, days: number = 30) {
    ...
    const avgLatency =
      totalRequests > 0
        ? Math.round(
            logs.reduce((s: number, l: { latencyMs: number | null }) => s + (l.latencyMs || 0), 0) /
              totalRequests
          )
        : 0;
  ...
  private buildAuthHeaders(server: {
    authType: string;
    ...
    try {
      ...
      if (!result.allowed) {
        const err: any = new Error(
          `Limite quotidienne atteinte (${result.currentCount}/${result.limit})`
        );
        err.statusCode = 429;
        err.quota = {
          limit: result.limit,
