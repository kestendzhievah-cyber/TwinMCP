 ...
 */
export async function validateAuth(authHeader: string | null): Promise<
  { valid: true; userId: string; email?: string } | { valid: false; error: string }
> {
...
async function _validateApiKeyHash(apiKey: string): Promise<
  { valid: true; userId: string; tier?: string } | { valid: false; error: string }
> {
  try {
