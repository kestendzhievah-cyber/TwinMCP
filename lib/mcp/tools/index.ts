...
export async function initializeTools(services: any = {}): Promise<void> {
  ...
  if (libraryResolutionService) {
    try {
      ...
      logger.info(
        `Registered TwinMCP tool: ${resolveLibraryTool.name} (${resolveLibraryTool.category})`
      );
    } catch (error) {
  ...
  logger.info(
    `Registry initialized: ${stats.totalTools} tools, ${schemasRegistered} schemas, ${stats.asyncTools} async`
  );
}
