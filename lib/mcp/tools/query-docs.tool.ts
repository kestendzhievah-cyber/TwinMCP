    ...
    try {
      ...
      try {
        ...
        if (cached) {
          cacheHit = true;
          logger.debug(
            `[QueryDocs] Cache hit for "${validatedInput.library_id}": "${validatedInput.query}"`
          );
          return {
            success: true,
            data: cached,
      ...
      // Log de la requête
      logger.debug(
        `[QueryDocs] Processing query for library "${validatedInput.library_id}": "${validatedInput.query}"`
      );
      ...
      // Log du résultat
      logger.debug(
        `[QueryDocs] Found ${result.results.length} results in ${Date.now() - startTime}ms`
      );
