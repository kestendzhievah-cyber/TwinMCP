import { MCPTool, Plugin, ToolFilters, ValidationResult, ExecutionResult } from './types';
...
export class MCPRegistry {
  ...
  register(tool: MCPTool): void {
    ...
    if (this.tools.size >= this.maxTools) {
      throw new Error(
        `Registry is full (max ${this.maxTools} tools). Unregister tools before adding new ones.`
      );
    }
