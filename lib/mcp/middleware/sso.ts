...
export class SSOService {
  ...
  registerSAMLProvider(id: string, name: string, config: SAMLConfig): SSOProvider {
  ...
  registerOIDCProvider(id: string, name: string, config: OIDCConfig): SSOProvider {
    const provider: SSOProvider = {
      id,
      name,
      type: 'oidc',
  ...
  createSAMLAuthnRequest(providerId: string): { redirectUrl: string; requestId: string } | null {
    ...
    if (provider?.type !== 'saml' || !provider.enabled) return null;
  ...
  processSAMLResponse(
    ...
    try {
      ...
      const email =
        this.extractXMLValue(xml, config.attributeMapping.email) ||
  ...
  createSLORequest(sessionId: string): { redirectUrl: string } | null {
    ...
    if (provider?.type !== 'saml') return null;
