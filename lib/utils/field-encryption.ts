...
function getEncryptionKey(): Buffer {
  ...
  if (!key) {
    throw new Error(
      'FIELD_ENCRYPTION_KEY or ENCRYPTION_KEY environment variable is required for field encryption'
    );
  }
...
export function isEncryptedField(value: string): boolean {
  try {
    ...
    return buf.length >= IV_LENGTH + AUTH_TAG_LENGTH + 1 && value === buf.toString('base64'); // valid base64 roundtrip
  } catch {
    return false;
  }
}
