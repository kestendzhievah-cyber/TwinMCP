...
describe('StdioMCPServer', () => {
  ...
  beforeEach(() => {
    ...
    // Créer le serveur
    server = new StdioMCPServer({ tools: [mockTool] });

    // Remplacer les streams par des mocks
    (server as any).input = mockInput;
    (server as any).output = mockOutput;
  });
  ...
  describe('Error Handling', () => {
    test('should handle tool execution errors', async () => {
      ...
      const response = JSON.parse(receivedMessages[1]);
      expect(response.error).toBeDefined();
      expect(response.error.code).toBe(-32603); // Tool execution error
    });
  });
});
