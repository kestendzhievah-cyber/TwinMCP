    ...
    return (<div className="flex h-screen w-screen bg-[#0a0a0f] text-gray-100 overflow-hidden">
      ...
      <aside className={(0, utils_1.cn)("bg-gradient-to-b from-[#1a0b2e] to-[#090311] border-r border-purple-700/30 backdrop-blur-xl relative flex flex-col transition-all duration-300 ease-in-out", isHovered ? "w-64" : "w-16")} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        <nav className="flex-1 px-4 space-y-2">
          <SidebarItem icon={<lucide_react_1.Home size={20}/>} label="Accueil" active={isHovered} onClick={() => handleNavigation('/dashboard')}/>
          <SidebarItem icon={<lucide_react_1.Users size={20}/>} label="Agents" active={isHovered} onClick={() => handleNavigation('/dashboard/agent-builder')}/>
          <SidebarItem icon={<lucide_react_1.BarChart3 size={20}/>} label="Statistiques" active={isHovered} onClick={() => handleNavigation('/dashboard/analytics')}/>
