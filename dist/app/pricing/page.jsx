...
function PricingPage() {
    const [billingPeriod, setBillingPeriod] = (0, react_1.useState)('monthly');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-lg p-1">
            <button onClick={() => setBillingPeriod('monthly')} className={`px-6 py-2 rounded-md transition ${billingPeriod === 'monthly'
            ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white'
            : 'text-gray-400 hover:text-white'}`}>
              Mensuel
            </button>
            <button onClick={() => setBillingPeriod('yearly')} className={`px-6 py-2 rounded-md transition ${billingPeriod === 'yearly'
            ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white'
            : 'text-gray-400 hover:text-white'}`}>
              Annuel
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {plans.map((plan) => (<div key={plan.id} className={`relative p-8 rounded-2xl border transition-all hover:scale-105 ${plan.popular
              ...
              <div className="text-center mb-6">
                ...
                {getSavings(plan) && (<div className="text-green-400 text-sm font-semibold">
                    {getSavings(plan)}
                  </div>)}
              </div>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature, index) => (<li key={index} className="flex items-start">
                    <lucide_react_1.Check className="w-5 h-5 text-green-400 mr-3 flex-shrink-0 mt-0.5"/>
                    <span className="text-gray-300">{feature}</span>
                  </li>))}
                {plan.limitations.length > 0 && (<>
                    <li className="pt-3 border-t border-slate-700"/>
                    {plan.limitations.map((limitation, index) => (<li key={index} className="flex items-start">
                        <lucide_react_1.X className="w-5 h-5 text-red-400 mr-3 flex-shrink-0 mt-0.5"/>
                        <span className="text-gray-500">{limitation}</span>
                      </li>))}
                  </>)}
              </ul>

              <button onClick={() => handleSubscribe(plan.id)} disabled={isLoading} className={`w-full py-3 rounded-lg font-semibold transition flex items-center justify-center ${plan.popular
        ...
        ].map((faq, index) => (<div key={index} className="p-6 bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl">
