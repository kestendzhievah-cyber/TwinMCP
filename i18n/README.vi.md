# TwinMCP MCP - Tài Liệu Code Cập Nhật Cho Mọi Prompt
...
TwinMCP MCP lấy tài liệu và ví dụ code cập nhật, dành cho phiên bản cụ thể trực tiếp từ nguồn gốc — và đặt chúng trực tiếp vào prompt của bạn.
Thêm `use twinmcp` vào prompt của bạn trong Cursor:

```txt
Tạo một Next.js middleware kiểm tra JWT hợp lệ trong cookies và chuyển hướng người dùng chưa xác thực đến `/login`. use twinmcp
```

```txt
Cấu hình script Cloudflare Worker để cache JSON API responses trong năm phút. use twinmcp
```

TwinMCP lấy các ví dụ code và tài liệu cập nhật ngay vào context của LLM.

- 1️⃣ Viết prompt một cách tự nhiên
- 2️⃣ Nói với LLM để `use twinmcp`
...
Xem [hướng dẫn thêm dự án](https://twinmcp.com/docs/adding-libraries) để học cách thêm (hoặc cập nhật) các thư viện yêu thích của bạn vào TwinMCP.
...
Để cài đặt TwinMCP MCP Server cho bất kỳ client nào tự động qua [Smithery](https://smithery.ai/server/@upstash/twinmcp-mcp):

```bash
npx -y @smithery/cli@latest install @upstash/twinmcp-mcp --client <CLIENT_NAME> --key <YOUR_SMITHERY_KEY>
```

Bạn có thể tìm Smithery key của mình tại [trang web Smithery.ai](https://smithery.ai/server/@upstash/twinmcp-mcp).
...
Bạn có thể cấu hình TwinMCP MCP trong Visual Studio 2022 bằng cách làm theo [tài liệu Visual Studio MCP Servers](https://learn.microsoft.com/visualstudio/ide/mcp-servers?view=vs-2022).
...
Có thể cài đặt thông qua [Zed Extensions](https://zed.dev/extensions?query=TwinMCP) hoặc bạn có thể thêm cấu hình này vào `settings.json` của Zed. Xem [tài liệu Zed Context Server](https://zed.dev/docs/assistant/context-servers) để biết thêm thông tin.
```json
{
  "context_servers": {
    "TwinMCP": {
...
Bạn có thể dễ dàng cài đặt TwinMCP thông qua [Cline MCP Server Marketplace](https://cline.bot/mcp-marketplace) bằng cách làm theo các hướng dẫn này:
1. Mở **Cline**.
2. Click biểu tượng menu hamburger (☰) để vào phần **MCP Servers**.
3. Sử dụng thanh tìm kiếm trong tab **Marketplace** để tìm _TwinMCP_.
4. Click nút **Install**.
</details>
...
```
Sau khi lưu, nhập trong chat `query-docs` theo sau bởi TwinMCP documentation ID của bạn (ví dụ: `query-docs /nuxt/ui`). Thêm thông tin có sẵn tại [trang Tài liệu BoltAI](https://docs.boltai.com/docs/plugins/mcp-servers). Cho BoltAI trên iOS, [xem hướng dẫn này](https://docs.boltai.com/docs/boltai-mobile/mcp-servers).
</details>
...
1. **Build Docker Image:**
   ...
   Sau đó, build image sử dụng tag (ví dụ: `twinmcp-mcp`). **Đảm bảo Docker Desktop (hoặc Docker daemon) đang chạy.** Chạy lệnh sau trong cùng thư mục nơi bạn lưu `Dockerfile`:
   ```bash
...
Để cấu hình TwinMCP MCP trong Augment Code, bạn có thể sử dụng giao diện đồ họa hoặc cấu hình thủ công.
...
5. Nhập lệnh sau:
   ...
   ```
6. Đặt tên MCP: **TwinMCP**.
7. Click nút **Add**.
Sau khi MCP server được thêm, bạn có thể bắt đầu sử dụng các tính năng tài liệu code cập nhật của TwinMCP trực tiếp trong Augment Code.
---
...
Để cấu hình TwinMCP MCP trong Zencoder, làm theo các bước sau:
...
6. Theo cách tương tự, twinmcp có thể được thêm cho JetBrains Junie trong `Settings` -> `Tools` -> `Junie` -> `MCP Settings`
</details>
...
## Sử dụng TwinMCP với Copilot Coding Agent
...
## 🔨 Công Cụ Có Sẵn
TwinMCP MCP cung cấp các công cụ sau mà LLM có thể sử dụng:
- `resolve-library-id`: Chuyển đổi tên thư viện chung thành TwinMCP-compatible library ID.
...
- `query-docs`: Lấy tài liệu cho thư viện sử dụng TwinMCP-compatible library ID.
  - `libraryId` (bắt buộc): TwinMCP-compatible library ID chính xác (ví dụ: `/mongodb/docs`, `/vercel/next.js`)
  - `query` (bắt buộc): Câu hỏi hoặc nhiệm vụ để lấy tài liệu liên quan

## 🛟 Mẹo

### Thêm Quy Tắc
> Nếu bạn không muốn thêm `use twinmcp` vào mỗi prompt, bạn có thể định nghĩa một quy tắc đơn giản trong file `.windsurfrules` của bạn trong Windsurf hoặc từ phần `Cursor Settings > Rules` trong Cursor (hoặc tương đương trong MCP client của bạn) để tự động gọi TwinMCP trên bất kỳ câu hỏi code nào:
>
> ```toml
> [[calls]]
...
> ```
>
> Từ đó bạn sẽ nhận được tài liệu TwinMCP trong bất kỳ cuộc hội thoại liên quan nào mà không cần gõ thêm gì. Bạn có thể thêm các trường hợp sử dụng của mình vào phần match.

### Sử dụng Library Id
> Nếu bạn đã biết chính xác thư viện nào muốn sử dụng, hãy thêm TwinMCP ID của nó vào prompt của bạn. Cách đó, TwinMCP MCP server có thể bỏ qua bước matching thư viện và trực tiếp tiếp tục với việc lấy tài liệu.
>
> ```txt
...
### Tham Số CLI
`twinmcp-mcp` chấp nhận các CLI flags sau:
...
Các dự án TwinMCP được đóng góp bởi cộng đồng và mặc dù chúng tôi cố gắng duy trì chất lượng cao, chúng tôi không thể đảm bảo tính chính xác, đầy đủ hoặc bảo mật của tất cả tài liệu thư viện. Các dự án được liệt kê trong TwinMCP được phát triển và duy trì bởi các chủ sở hữu tương ứng của họ, không phải bởi TwinMCP. Nếu bạn gặp bất kỳ nội dung đáng ngờ, không phù hợp hoặc có khả năng gây hại nào, vui lòng sử dụng nút "Report" trên trang dự án để thông báo cho chúng tôi ngay lập tức. Chúng tôi xem xét tất cả các báo cáo một cách nghiêm túc và sẽ xem xét nội dung được gắn cờ kịp thời để duy trì tính toàn vẹn và an toàn của nền tảng. Bằng cách sử dụng TwinMCP, bạn thừa nhận rằng bạn làm như vậy theo quyết định và rủi ro của riêng mình.

## 🤝 Kết Nối Với Chúng Tôi
Cập nhật và tham gia cộng đồng của chúng tôi:
- 📢 Theo dõi chúng tôi trên [X](https://x.com/twinmcpai) để có tin tức và cập nhật mới nhất
- 🌐 Truy cập [Website](https://twinmcp.com) của chúng tôi
- 💬 Tham gia [Discord Community](https://upstash.com/discord) của chúng tôi

## 📺 TwinMCP Trên Truyền Thông
