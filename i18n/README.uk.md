# TwinMCP MCP — Актуальна документація з прикладами коду для будь-якого запиту
...
TwinMCP MCP отримує актуальну, специфічну для версії документацію та приклади коду безпосередньо з джерела — і вбудовує їх прямо у ваш промпт.
Додайте `use twinmcp` до вашого запиту в Cursor:

```txt
Create a Next.js middleware that checks for a valid JWT in cookies and redirects unauthenticated users to `/login`. use twinmcp
```

```txt
Configure a Cloudflare Worker script to cache JSON API responses for five minutes. use twinmcp
```

TwinMCP завантажує свіжі приклади коду й документацію безпосередньо в контекст вашої великої мовної моделі.

- 1️⃣ Написуйте ваш промпт природно
- 2️⃣ Скажіть ШІ використати `use twinmcp`
- 3️⃣ Отримайте робочі відповіді з кодом
  Без перемикання між вкладками, без неіснуючих API та без застарілого коду.

## 📚 Додавання проєктів

Ознайомтеся з нашим [посібником з додавання проєктів](https://twinmcp.com/docs/adding-libraries), щоб дізнатися, як додати (або оновити) ваші улюблені бібліотеки в TwinMCP.
...
Для автоматичного встановлення TwinMCP MCP Server для будь-якого клієнта через [Smithery](https://smithery.ai/server/@upstash/twinmcp-mcp):

```bash
npx -y @smithery/cli@latest install @upstash/twinmcp-mcp --client <CLIENT_NAME> --key <YOUR_SMITHERY_KEY>
```

Ваш ключ Smithery можна знайти на [сторінці Smithery.ai](https://smithery.ai/server/@upstash/twinmcp-mcp).
...
[<img alt="Встановити в VS Code (npx)" src="https://img.shields.io/badge/VS_Code-VS_Code?style=flat-square&label=Встановити%20TwinMCP%20MCP&color=0098FF">](https://insiders.vscode.dev/redirect?url=vscode%3Amcp%2Finstall%3F%7B%22name%22%3A%22twinmcp%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40upstash%2Ftwinmcp-mcp%40latest%22%5D%7D)
[<img alt="Встановити в VS Code Insiders (npx)" src="https://img.shields.io/badge/VS_Code_Insiders-VS_Code_Insiders?style=flat-square&label=Встановити%20TwinMCP%20MCP&color=24bfa5">](https://insiders.vscode.dev/redirect?url=vscode-insiders%3Amcp%2Finstall%3F%7B%22name%22%3A%22twinmcp%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40upstash%2Ftwinmcp-mcp%40latest%22%5D%7D)
...
Ви можете налаштувати TwinMCP MCP у Visual Studio 2022, дотримуючись [документації Visual Studio MCP Servers](https://learn.microsoft.com/visualstudio/ide/mcp-servers?view=vs-2022).
...
Можна встановити через [розширення Zed](https://zed.dev/extensions?query=TwinMCP) або додати це до вашого `settings.json`. Детальніше див. у [документації Zed Context Server](https://zed.dev/docs/assistant/context-servers).
```json
{
  "context_servers": {
    "TwinMCP": {
...
## Використання TwinMCP з Copilot Coding Agent
...
Ви можете легко встановити TwinMCP через [торговий майданчик MCP-серверів Cline](https://cline.bot/mcp-marketplace), дотримуючись цих інструкцій:
1. Відкрийте **Cline**.
2. Натисніть значок меню гамбургер (☰), щоб увійти до розділу **MCP Servers**.
3. Використовуйте панель пошуку у вкладці **Marketplace**, щоб знайти _TwinMCP_.
...
```
Після збереження введіть у чаті `query-docs`, а потім ваш ідентифікатор документації TwinMCP (наприклад, `query-docs /nuxt/ui`). Додаткова інформація доступна на [сайті документації BoltAI](https://docs.boltai.com/docs/plugins/mcp-servers). Для BoltAI на iOS [див. цей посібник](https://docs.boltai.com/docs/boltai-mobile/mcp-servers).
</details>
...
1. **Створіть Docker-образ:**
   ...
   Потім створіть образ, використовуючи тег (наприклад, `twinmcp-mcp`). **Переконайтеся, що Docker Desktop (або демон Docker) запущений.** Виконайте наступну команду в тій же теці, де ви зберегли `Dockerfile`:
   ```bash
...
Для налаштування TwinMCP MCP в Augment Code ви можете використовувати або графічний інтерфейс, або ручну конфігурацію.
...
Для налаштування TwinMCP MCP в Zencoder виконайте наступні кроки:
...
TwinMCP MCP надає наступні інструменти, які можуть використовувати великі мовні моделі:
- `resolve-library-id`: Перетворює загальну назву бібліотеки на сумісний з TwinMCP ідентифікатор бібліотеки.
...
- `query-docs`: Отримує документацію для бібліотеки, використовуючи сумісний з TwinMCP ідентифікатор бібліотеки.
  - `libraryId` (обов'язково): Точний сумісний з TwinMCP ідентифікатор бібліотеки (наприклад, `/mongodb/docs`, `/vercel/next.js`)
  - `query` (обов'язково): Питання або завдання для отримання релевантної документації

## 🛟 Поради

### Додайте правило
> Якщо ви не хочете додавати `use twinmcp` до кожного промпту, ви можете визначити просте правило у вашому файлі `.windsurfrules` в Windsurf або в розділі `Cursor Settings > Rules` в Cursor (або еквівалентному у вашому MCP-клієнті), щоб автоматично викликати TwinMCP для будь-яких запитань про код:
>
> ```toml
> [[calls]]
...
> ```
>
> Відтоді ви отримуватимете документацію TwinMCP у будь-якій пов'язаній розмові без введення будь-чого додаткового. Ви можете додати свої випадки використання до частини match.

### Використовуйте ідентифікатор бібліотеки
> Якщо ви вже точно знаєте, яку бібліотеку хочете використовувати, додайте її ідентифікатор TwinMCP до вашого промпту. Таким чином TwinMCP MCP-сервер може пропустити крок пошуку бібліотеки та одразу перейти до отримання документації.
>
> ```txt
...
`twinmcp-mcp` приймає наступні прапори CLI:
...
## ⚠️ Застереження
Проєкти TwinMCP створюються спільнотою, і хоча ми прагнемо підтримувати високу якість, ми не можемо гарантувати точність, повноту або безпеку всієї документації бібліотек. Проєкти, перелічені в TwinMCP, розробляються та підтримуються їхніми відповідними власниками, а не TwinMCP. Якщо ви зіткнетеся з будь-яким підозрілим, неприйнятним або потенційно шкідливим контентом, будь ласка, використовуйте кнопку "Report" на сторінці проєкту, щоб негайно повідомити нас. Ми серйозно ставимося до всіх звітів і оперативно переглядаємо позначений контент для підтримання цілісності та безпеки нашої платформи. Використовуючи TwinMCP, ви визнаєте, що робите це на власний розсуд і ризик.
...
- 🌐 Відвідайте наш [веб-сайт](https://twinmcp.com)
...
## ⭐ Історія зірок
[![Діаграма історії зірок](https://api.star-history.com/svg?repos=upstash/twinmcp&type=Date)](https://www.star-history.com/#upstash/twinmcp&Date)
