...
export async function librarySearchRoutes(fastify: FastifyInstance) {
  ...
  const rateLimitMiddleware = new SearchRateLimitMiddleware(f.redis);
  const controller = new LibrarySearchController(
    f.librarySearchService,
    f.libraryAnalyticsService,
    f.cacheService
  );

  // Recherche principale
  fastify.get('/api/libraries/search', {
    preHandler: [rateLimitMiddleware.search as any],
    schema: {
      querystring: {
        type: 'object',
        properties: {
  ...
  fastify.get('/api/libraries/autocomplete', {
    preHandler: [rateLimitMiddleware.autocomplete as any],
    schema: {
      querystring: {
        type: 'object',
        properties: {
  ...
  fastify.get('/api/libraries/:name', {
    preHandler: [rateLimitMiddleware.libraryDetail as any],
    schema: {
      params: {
        type: 'object',
        properties: {
  ...
  fastify.post('/api/libraries/suggestions', {
    preHandler: [authMiddleware.optional, rateLimitMiddleware.suggestions as any],
    schema: {
      body: {
        type: 'object',
        properties: {
  ...
  fastify.get('/api/libraries/export', {
    preHandler: [authMiddleware.required, rateLimitMiddleware.exportLimit as any],
    schema: {
      querystring: {
        type: 'object',
        properties: {
