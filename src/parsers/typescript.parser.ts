...
export class TypeScriptParser implements ParserInterface {
  async parse(content: string, options: ParseOptions): Promise<ParserResult> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine.startsWith('/**')) {
        ...
        // Détection des sections principales (classes, interfaces, fonctions)
    ...
    const mainCodeBlock: CodeBlock = {
      ...
      language: 'typescript',
      code: content,
      context: 'TypeScript source file',
      position: {
        start: 0,
        end: content.length
      },
    ...
    sections.forEach(section => {
      ...
      section.metadata.hasCode = /function|class|interface|type|const|let|var/.test(section.content);
    ...
    return {
      ...
      content: this.cleanTypeScriptContent(content),
      rawContent: content
    };
  }
  ...
  private isSectionHeader(line: string): boolean {
    const sectionPatterns = [
      /^(export\s+)?(class|interface|type|enum|namespace)\s+\w+/,
  ...
  private extractSectionTitle(line: string): string {
    const classMatch = line.match(/(?:class|interface|type|enum|namespace)\s+(\w+)/);
  ...
  private extractJSDocTitle(comment: string): string {
    const titleMatch = comment.match(/\*\s+@(?:class|function|interface|description)\s+(.+)/);
  ...
  private cleanTypeScriptContent(content: string): string {
    return content
  ...
  private extractTitle(content: string, filePath: string): string {
    ...
    const exportMatches = content.match(/export\s+(?:default\s+)?(?:class|function|interface)\s+(\w+)/);
