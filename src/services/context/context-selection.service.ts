...
Respond in JSON format.`
      ...
      metadata: { purpose: 'documentation' as any, priority: 'normal' as any }
    });
  ...
  private async searchRelevantDocs(
    ...
    const results = await (prisma as any).crawledContent.findMany({
      where: {
        OR: [
  ...
  private async getRelevantHistory(
    ...
    const embeddingResults = await this.embeddingService.generateEmbeddings({
      chunks: [{ id: 'query', content: query, metadata: { url: '', title: '', contentType: 'text', position: 0, totalChunks: 1, lastModified: new Date() } as any }],
