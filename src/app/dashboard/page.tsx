...
export default function DashboardNeon() {
  ...
  const stats = [
    { icon: Bot, label: "Agents Actifs", value: `${userPlan.agentsUsed}/${userPlan.agentsLimit}`, trend: "+2 ce mois", trendUp: true, color: "violet" },
    { icon: MessageSquare, label: "Conversations", value: userPlan.conversationsUsed.toLocaleString(), trend: `${((userPlan.conversationsUsed / userPlan.conversationsLimit) * 100).toFixed(1)}% utilisé`, trendUp: false, color: "blue" },
    { icon: DollarSign, label: "Coût Total", value: `${userPlan.cost.toFixed(2)}€`, trend: "-12% vs mois dernier", trendUp: true, color: "green" },
    { icon: TrendingUp, label: "Satisfaction", value: "4.77/5", trend: "+0.3 ce mois", trendUp: true, color: "pink" },
  ];

  const quickActions = [
    { icon: Plus, title: "Nouveau Template", description: "Démarrez depuis un template", href: "/dashboard/templates" },
    { icon: BarChart3, title: "Voir Analytics", description: "Explorer les performances" , href: "/dashboard/analytics"},
    { icon: Zap, title: "Optimiser", description: "Réduire les coûts", href: "/dashboard/optimize" },
  ];
  ...
  const generateSectionClass = (sectionName: string) => {
  ...
  const generateCardClass = (agentId: string) => {
      ...
      <div className="px-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={i} className="p-6 rounded-2xl bg-[rgba(255,255,255,0.03)] border border-white/6 backdrop-blur-lg relative overflow-hidden hover:scale-[1.02] transition-transform" style={{ boxShadow: neonGlow(s.color) }}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className={`text-sm ${s.trendUp ? 'text-green-400' : 'text-white/50'} flex items-center gap-1`}>
                    {s.trendUp ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />}
                    ...
                    return (
                      <g key={i} className="group">
                        ...
                        <rect
                          ...
                          width="50"
                          height={barHeight}
                          fill="url(#full-bar-gradient)"
                          rx="6"
                          filter="url(#full-glow)"
                        ...
                        <circle
                          ...
                          cy={y - 30}
                          r="15"
                          fill={agent.type === 'support' ? '#0ea5e9' : agent.type === 'sales' ? '#10b981' : '#8b5cf6'}
                          className="cursor-pointer transition-all duration-300 hover:scale-110 hover:brightness-125"
                          filter="url(#full-glow)"
                        />
        ...
        <section className={generateSectionClass("agents-list")}>
          ...
          <div className="grid gap-6">
            {agents.map((a) => (
              <div
                key={a.id}
                className={`p-6 rounded-2xl bg-white/${dynamicStyles.backgroundOpacity} border border-white/${dynamicStyles.borderOpacity} flex items-start justify-between ${generateCardClass(a.id)}`}
                style={{
              ...
              >
                ...
                <div className="flex flex-col gap-2 ml-6">
                  <button className="p-2 rounded-md bg-white/5 hover:bg-white/8 transition"><BarChart3 className="w-4 h-4" /></button>
                  <button className="p-2 rounded-md bg-white/5 hover:bg-green-600 transition">{a.status==='active'? <Pause className="w-4 h-4" />: <Play className="w-4 h-4" />}</button>
                  <button className="p-2 rounded-md bg-white/5 hover:bg-white/8 transition"><Edit className="w-4 h-4" /></button>
                  <button className="p-2 rounded-md bg-white/5 hover:bg-red-600 transition"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className={generateSectionClass("quick-actions")}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {quickActions.map((q, i) => {
              const Icon = q.icon;
              return (
                <div key={i} className="p-6 rounded-2xl border border-white/6 bg-[rgba(255,255,255,0.02)] hover:scale-[1.03] transition" style={{ boxShadow: neonGlow(i===0? 'violet':'blue') }}>
                  <div className="flex items-center gap-4">
