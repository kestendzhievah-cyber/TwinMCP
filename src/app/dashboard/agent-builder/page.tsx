...
import { Send, Plus, Settings, Sparkles, Database, Mail, Calendar, Github, Slack, MessageSquare, X, Check } from 'lucide-react';
...
export default function AgentBuilder() {
  ...
  const [agentName, setAgentName] = useState('Mon Agent IA');
  const [connectedTools, setConnectedTools] = useState<string[]>(['email']);
  ...
  const availableTools = [
    { id: 'email', name: 'Email', icon: Mail, description: 'Envoyer et recevoir des emails', connected: true },
    { id: 'calendar', name: 'Google Calendar', icon: Calendar, description: 'Gérer les événements', connected: false },
    { id: 'github', name: 'GitHub', icon: Github, description: 'Accès aux repositories', connected: false },
    { id: 'slack', name: 'Slack', icon: Slack, description: 'Envoyer des messages', connected: false },
    { id: 'database', name: 'Database', icon: Database, description: 'Accès aux données', connected: false },
    { id: 'crm', name: 'CRM', icon: MessageSquare, description: 'Gérer les contacts', connected: false }
  ];
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <input 
                ...
                className="text-2xl font-bold text-white bg-transparent border-none outline-none mb-2"
              />
              <p className="text-gray-400">Configurez et testez votre agent IA</p>
            </div>
            <button 
              onClick={handleDeployAgent}
              className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-600 transition shadow-lg shadow-purple-500/50"
            >
              Déployer l'Agent
            </button>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
                ...
                {availableModels.map((model) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {selectedModel === model.id && <Check className="w-5 h-5 text-purple-400" />}
                    </div>
              ...
              <div className="space-y-2">
                {connectedTools.length === 0 ? (
                  ...
                  connectedTools.map((toolId) => {
                    ...
                    if (!tool) return null;
                    return (
                      <div key={toolId} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
            ...
            <div className="p-6 bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl">
              ...
              <textarea 
                className="w-full h-32 bg-slate-700/50 text-white rounded-lg p-3 border border-slate-600 focus:border-purple-500 outline-none resize-none"
                placeholder="Décrivez le comportement de votre agent..."
                defaultValue="Tu es un assistant commercial expert qui aide les clients à trouver le bon produit."
              />
            </div>
          </div>
              ...
              <div className="p-4 border-t border-purple-500/20">
                <div className="flex space-x-2">
                  <input 
                    ...
                    className="flex-1 bg-slate-700/50 text-white rounded-lg px-4 py-3 border border-slate-600 focus:border-purple-500 outline-none"
                  />
                  <button 
                    onClick={handleSendMessage}
                    className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition shadow-lg shadow-purple-500/50"
                  >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl border border-purple-500/20 max-w-4xl w-full max-h-[80vh] overflow-y-auto">
              ...
              {availableTools.map((tool) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {connectedTools.includes(tool.id) && (
                      <div className="px-2 py-1 bg-green-500/20 text-green-400 text-xs font-semibold rounded border border-green-500/30">
                        Connecté
                      </div>
                    )}
                  </div>
            ...
            <div className="p-6 border-t border-purple-500/20">
              <button 
                onClick={() => setShowToolsModal(false)}
                className="w-full px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-600 transition"
              >
                Confirmer
              </button>
            </div>
