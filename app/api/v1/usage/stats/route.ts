...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get usage by tool
    const toolUsage = new Map<string, { count: number; success: number; avgTime: number }>();
    logs.forEach(log => {
      const existing = toolUsage.get(log.toolName) || { count: 0, success: 0, totalTime: 0 };
      existing.count++;
      if (log.success) existing.success++;
      if (log.responseTimeMs) existing.totalTime = (existing.totalTime || 0) + log.responseTimeMs;
    ...
    const toolBreakdown = Array.from(toolUsage.entries()).map(([tool, data]) => ({
      ...
      avgResponseTime: Math.round((data.totalTime || 0) / data.count)
    }));
