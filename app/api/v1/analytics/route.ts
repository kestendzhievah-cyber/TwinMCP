...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalTokens = usageLogs.reduce((sum, log) => sum + (log.tokensReturned || 0), 0);
    const avgResponseTime = totalRequests > 0
      ? Math.round(usageLogs.reduce((sum, log) => sum + (log.responseTimeMs || 0), 0) / totalRequests)
      : 0;
    ...
    for (let i = 0; i < timeSlots; i++) {
      ...
      const slotLogs = usageLogs.filter(log => {
      ...
      usageOverTime.push({
        ...
        tokens: slotLogs.reduce((sum, log) => sum + (log.tokensReturned || 0), 0)
      });
    }
