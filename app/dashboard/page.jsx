...
exports.default = TwinMCPApiDocs;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const api_keys_client_1 = require("@/lib/api-keys-client");
...
function tryParseMcpConfig(text) {
    try {
        ...
        const normalized = {};
        for (const [id, cfg] of Object.entries(mcpServersRaw)) {
            ...
            const headers = headersRaw && typeof headersRaw === 'object'
                ? Object.fromEntries(Object.entries(headersRaw)
...
function getServerLogoUrl(_id, url) {
    try {
...
function TwinMCPApiDocs() {
    const [showOverviewApiKey, setShowOverviewApiKey] = (0, react_1.useState)(false);
    const [copied, setCopied] = (0, react_1.useState)(false);
    const [selectedTab, setSelectedTab] = (0, react_1.useState)('overview');
    const [selectedLang, setSelectedLang] = (0, react_1.useState)('nodejs');
    const [logoErrors, setLogoErrors] = (0, react_1.useState)({});
    ...
    const [mcpServerId, setMcpServerId] = (0, react_1.useState)("context7");
    const [mcpServerUrl, setMcpServerUrl] = (0, react_1.useState)("https://mcp.context7.com/mcp");
    const [mcpHeaderKey, setMcpHeaderKey] = (0, react_1.useState)("CONTEXT7_API_KEY");
    const [mcpHeaderValue, setMcpHeaderValue] = (0, react_1.useState)("");
    const [mcpImportText, setMcpImportText] = (0, react_1.useState)(JSON.stringify({
        mcpServers: {
            context7: {
    ...
    const [mcpUiMessage, setMcpUiMessage] = (0, react_1.useState)(null);
    const [mcpUiError, setMcpUiError] = (0, react_1.useState)(null);
    const [mcpHasLoadedFromStorage, setMcpHasLoadedFromStorage] = (0, react_1.useState)(false);
    ...
    const [itemsPerPage] = (0, react_1.useState)(5);
    const [showStats, setShowStats] = (0, react_1.useState)(false);
    const [isLoading, setIsLoading] = (0, react_1.useState)(false);
    const [newCreatedKey, setNewCreatedKey] = (0, react_1.useState)(null);
    ...
    const loadApiKeys = async () => {
        setIsLoading(true);
        try {
            const keys = await api_keys_client_1.apiKeysClient.getApiKeys();
            setApiKeys(keys);
        }
        catch (error) {
    ...
    const createApiKey = async () => {
        ...
        setIsLoading(true);
        try {
            const newKey = await api_keys_client_1.apiKeysClient.createApiKey(newKeyName.trim());
    ...
    const deleteApiKey = async (keyId) => {
        ...
        setIsLoading(true);
        try {
            await api_keys_client_1.apiKeysClient.revokeApiKey(keyId);
    ...
    (0, react_1.useEffect)(() => {
        if (process.env.NODE_ENV === 'development') {
            api_keys_client_1.apiKeysClient.simulateAdminAuth();
        }
        loadApiKeys();
    }, []);
    // Export API keys
    ...
    const filteredApiKeys = (0, react_1.useMemo)(() => {
        let filtered = apiKeys;
        // Filter by search term
        if (searchTerm) {
            filtered = filtered.filter(key => key.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                key.keyPrefix.toLowerCase().includes(searchTerm.toLowerCase()));
        }
        // Filter by tag (simulé pour l'instant)
        if (selectedTag !== 'all') {
            filtered = filtered.filter(key => key.name?.includes(selectedTag));
        }
        return filtered;
    ...
    const allTags = (0, react_1.useMemo)(() => {
        ...
        apiKeys.forEach(key => {
            if (key.name) {
                ...
                const nameTags = key.name.split(/\s+/).filter(word => ['production', 'development', 'test', 'staging', 'demo'].includes(word.toLowerCase()));
    ...
    // Pagination
    const paginatedKeys = (0, react_1.useMemo)(() => {
    ...
    const statistics = (0, react_1.useMemo)(() => {
        ...
        const recentlyUsed = apiKeys.filter(key => key.lastUsedAt && new Date(key.lastUsedAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length;
        return { activeKeys, expiredKeys, totalUsage, recentlyUsed };
    }, [apiKeys]);
    const mcpConfigText = (0, react_1.useMemo)(() => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <lucide_react_1.Sparkles className="w-8 h-8 text-purple-400"/>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex space-x-1 mb-8 bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-lg p-1">
          {[
            { id: 'overview', label: 'Overview', icon: lucide_react_1.Book },
            { id: 'libraries', label: 'Libraries', icon: lucide_react_1.Code },
            { id: 'members', label: 'Members', icon: lucide_react_1.Zap },
            { id: 'rules', label: 'Rules', icon: lucide_react_1.Terminal }
        ].map((tab) => (<button key={tab.id} onClick={() => setSelectedTab(tab.id)} className={`flex-1 flex items-center justify-center space-x-2 px-4 py-3 rounded-md font-medium transition ${selectedTab === tab.id
        ...
        <section className="mb-8">
          <div className="p-6 rounded-2xl bg-[rgba(255,255,255,0.02)] border border-white/6 backdrop-blur" style={{ boxShadow: '0 0 10px rgba(255, 255, 255, 0.1)' }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <lucide_react_1.Zap className="w-6 h-6 text-purple-300"/>
                Connexion MCP
              </h2>
              <button className="px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-sm flex items-center gap-2" onClick={() => copyTextToClipboard(mcpConfigText)} title="Copier la configuration">
                <lucide_react_1.Copy className="w-4 h-4"/>
                Copier JSON
              </button>
            </div>

            {(mcpUiMessage || mcpUiError) && (<div className="mb-4">
              ...
              <div className="p-4 rounded-xl bg-white/3 border border-white/6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={mcpServerId} onChange={(e) => setMcpServerId(e.target.value)} className="w-full bg-white/6 rounded-lg px-3 py-2 outline-none border border-white/6" placeholder="context7"/>
                  </div>
                  <div>
                    <div className="text-xs text-white/60 mb-1">URL MCP</div>
                    <input value={mcpServerUrl} onChange={(e) => setMcpServerUrl(e.target.value)} className="w-full bg-white/6 rounded-lg px-3 py-2 outline-none border border-white/6" placeholder="https://mcp.context7.com/mcp"/>
                  </div>
                  <div>
                    <div className="text-xs text-white/60 mb-1">Header (clé)</div>
                    <input value={mcpHeaderKey} onChange={(e) => setMcpHeaderKey(e.target.value)} className="w-full bg-white/6 rounded-lg px-3 py-2 outline-none border border-white/6" placeholder="CONTEXT7_API_KEY"/>
                  </div>
                  <div>
                    <div className="text-xs text-white/60 mb-1">Header (valeur)</div>
                    <input value={mcpHeaderValue} onChange={(e) => setMcpHeaderValue(e.target.value)} className="w-full bg-white/6 rounded-lg px-3 py-2 outline-none border border-white/6" placeholder="YOUR_API_KEY"/>
                  </div>
                </div>

                <div className="mt-3 flex items-center gap-2">
                  <button className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#8b5cf6] to-[#ef4d95] font-semibold" onClick={handleAddOrUpdateMcpServer}>
                    Enregistrer
                  </button>
                  <button className="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10" onClick={() => {
            ...
            setMcpHeaderValue("");
                ...
                <div className="mt-5">
                  ...
                  {Object.keys(mcpServers).length === 0 ? (<div className="text-sm text-white/50">Aucun serveur MCP configuré</div>) : (<div className="space-y-2">
                      {Object.entries(mcpServers).map(([id, cfg]) => {
                const logo = getServerLogoUrl(id, cfg.url);
                const showFallback = !logo || !!logoErrors[id];
                return (<div key={id} className="flex items-center justify-between p-3 rounded-lg bg-white/4 border border-white/6">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-9 h-9 rounded-lg bg-white/6 border border-white/6 flex items-center justify-center overflow-hidden relative">
                                {logo && !logoErrors[id] && (<img src={logo} alt={`${id} logo`} className="w-full h-full object-contain" onError={(e) => {
                            e.currentTarget.style.display = "none";
                            setLogoErrors(prev => ({ ...prev, [id]: true }));
                        }}/>)}
                                {showFallback && (<div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-white/70">
                            ...
                            <div className="flex items-center gap-2">
                              <button className="px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-sm flex items-center gap-2" onClick={() => {
                        setMcpServerId(id);
                        ...
                        setMcpHeaderValue(hk && cfg.headers ? cfg.headers[hk] : "");
                              ...
                              <button className="px-3 py-2 rounded-lg bg-white/5 hover:bg-red-600/40 text-sm flex items-center gap-2" onClick={() => handleRemoveMcpServer(id)} title="Supprimer">
              ...
              <div className="p-4 rounded-xl bg-white/3 border border-white/6">
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button className="text-sm text-purple-300 hover:text-purple-200 flex items-center gap-2" onClick={() => copyTextToClipboard(mcpConfigText)}>
                      <lucide_react_1.Copy className="w-4 h-4"/>
                      Copier
                    </button>
                  </div>
                  <textarea value={mcpConfigText} readOnly rows={10} className="w-full bg-black/30 rounded-lg px-3 py-2 outline-none border border-white/10 font-mono text-xs"/>
                </div>

                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-xs text-white/60">Import</div>
                    <button className="text-sm text-purple-300 hover:text-purple-200" onClick={handleImportMcpConfig}>
                      Importer
                    </button>
                  </div>
                  <textarea value={mcpImportText} onChange={(e) => setMcpImportText(e.target.value)} rows={10} className="w-full bg-black/30 rounded-lg px-3 py-2 outline-none border border-white/10 font-mono text-xs"/>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <lucide_react_1.Key className="w-4 h-4 text-green-400"/>
            </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <lucide_react_1.BarChart className="w-4 h-4 text-blue-400"/>
            </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <lucide_react_1.Clock className="w-4 h-4 text-purple-400"/>
            </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <button onClick={exportApiKeys} className="px-4 py-2 bg-white/6 border border-white/10 text-white rounded-lg hover:bg-white/10 transition text-sm flex items-center">
          ...
          {isCreatingKey && (<div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-4 mb-6">
              ...
              {newCreatedKey ? (<div className="mb-4 p-3 bg-green-900/20 border border-green-500/30 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => copyApiKeyToClipboard(newCreatedKey, 'new')} className="p-2 text-green-400 hover:text-green-300 transition">
                      {copiedKey === 'new' ? <lucide_react_1.Check size={16}/> : <lucide_react_1.Copy size={16}/>}
                    </button>
                  </div>
                  <p className="text-yellow-300 text-xs mt-2">⚠️ Copiez cette clé maintenant, elle ne sera plus affichée ensuite.</p>
                </div>) : (<div className="flex gap-3">
                  <input type="text" value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} placeholder="Nom de la clé (ex: Production, Développement)" className="flex-1 px-4 py-2 bg-white/6 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none transition" disabled={isLoading} autoComplete="off" autoFocus/>
                  <button onClick={createApiKey} disabled={isLoading || !newKeyName.trim()} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                }} disabled={isLoading} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
                    Annuler
                  </button>
                </div>)}
            </div>)}
          ...
          {/* Loading indicator */}
          {isLoading && !isCreatingKey && (<div className="flex items-center justify-center py-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            setCurrentPage(1);
        }} placeholder="Rechercher par nom ou clé..." className="w-full pl-10 pr-4 py-2 bg-white/6 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none transition"/>
            </div>
            <div className="flex gap-2">
            ...
            setCurrentPage(1);
              ...
              <button onClick={() => setShowStats(!showStats)} className={`px-4 py-2 rounded-lg transition flex items-center gap-2 ${showStats
          ...
          <div className="space-y-3">
            {paginatedKeys.map((apiKey) => (<div key={apiKey.id} className="bg-white/3 border border-white/6 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {apiKey.usage && (<div className="text-xs text-blue-400">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <button onClick={() => copyApiKeyToClipboard(newCreatedKey || apiKey.keyPrefix, apiKey.id)} className="p-1 text-gray-400 hover:text-white transition">
                    ...
                    {apiKey.name && (<div className="flex items-center gap-2 mb-2">
                        {apiKey.name.split(/\s+/).filter(word => ['production', 'development', 'test', 'staging', 'demo'].includes(word.toLowerCase())).map((tag, index) => (<span key={index} className="px-2 py-1 text-xs bg-purple-500/20 text-purple-300 rounded">
                            <lucide_react_1.Tag className="w-3 h-3 inline mr-1"/>
                            {tag}
                          </span>))}
                      </div>)}
                    {apiKey.lastUsedAt && (<div className="text-xs text-gray-400">
                        Dernière utilisation: {new Date(apiKey.lastUsedAt).toLocaleString('fr-FR')}
                      </div>)}
                    {apiKey.usage && (<div className="text-xs text-gray-400 mt-1">
                        Taux de succès: {apiKey.usage.successRate.toFixed(1)}%
                      </div>)}
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => regenerateApiKey(apiKey.id)} className="p-2 text-blue-400 hover:text-blue-300 transition" title="Régénérer la clé">
          ...
          {totalPages > 1 && (<div className="flex items-center justify-center gap-2 mt-6">
              <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-3 py-2 bg-white/6 border border-white/10 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/10 transition">
                Précédent
              </button>
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (<button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-2 rounded-lg transition ${currentPage === page
                    ? 'bg-purple-600 text-white'
                    : 'bg-white/6 border border-white/10 text-white hover:bg-white/10'}`}>
                    {page}
                  </button>))}
              </div>
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-2 bg-white/6 border border-white/10 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/10 transition">
                Suivant
              </button>
            </div>)}
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6 mb-8">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button onClick={() => handleCopy('https://api.twinmcp.com/v1')} className="p-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
                  {copied ? <lucide_react_1.Check className="w-5 h-5"/> : <lucide_react_1.Copy className="w-5 h-5"/>}
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button onClick={() => handleCopy('twinmcp.com/api/v1')} className="p-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {['Cursor', 'Claude Code', 'VS Code', 'Coda', 'Windsurf', 'Qwen CLI'].map((tool) => (<button key={tool} className="px-3 py-2 bg-slate-700/50 border border-slate-600 text-gray-300 text-sm rounded-lg hover:border-purple-500/50 transition">
                {tool}
              </button>))}
      ...
      "tmcp_sk_YOUR_API_KEY"
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6">
          ...
          <div className="flex space-x-2 mb-4">
            ...
            { id: 'curl', label: 'cURL' }
        ].map((lang) => (<button key={lang.id} onClick={() => setSelectedLang(lang.id)} className={`px-4 py-2 text-sm font-medium rounded-lg transition ${selectedLang === lang.id
          ...
          <div className="relative mb-6">
            <div className="absolute top-3 right-3 z-10">
              <button onClick={() => handleCopy(codeExamples[selectedLang])} className="p-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
                {copied ? <lucide_react_1.Check className="w-4 h-4"/> : <lucide_react_1.Copy className="w-4 h-4"/>}
              </button>
            </div>
            <div className="bg-slate-900 border border-purple-500/20 rounded-lg p-4 overflow-x-auto">
