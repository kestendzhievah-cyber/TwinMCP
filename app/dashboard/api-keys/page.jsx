...
function ApiKeysPage() {
    ...
    const [creatingKey, setCreatingKey] = (0, react_1.useState)(false);
    const [newApiKey, setNewApiKey] = (0, react_1.useState)(null);
    const [copiedKey, setCopiedKey] = (0, react_1.useState)(null);
    const [revokingKey, setRevokingKey] = (0, react_1.useState)(null);
    ...
    (0, react_1.useEffect)(() => {
        const loadApiKeys = async () => {
            ...
            try {
                ...
                const result = await api_client_1.apiClient.getApiKeys();
                if (result.success) {
    ...
    const handleCreateKey = async () => {
        ...
        try {
            // Appel API réel via notre client
            const result = await api_client_1.apiClient.createApiKey(newKeyName.trim());
            if (result.success) {
                setNewApiKey({
    ...
    const handleRevokeKey = async (keyId) => {
        ...
        try {
            // Appel API réel via notre client
            const result = await api_client_1.apiClient.revokeApiKey(keyId);
            if (result.success) {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-[#0a0118] via-[#1a0b2e] to-[#0f0520] text-white relative overflow-hidden">
      {/* Background Effects */}
      ...
      <div className="relative z-10">
            ...
            <div className="flex justify-between items-center h-16">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-pink-500 to-purple-600 shadow-lg shadow-pink-500/30">
                  <lucide_react_1.Key className="w-6 h-6 text-white"/>
                </div>
                <div>
              ...
              <button onClick={() => setShowCreateModal(true)} className="px-5 py-2.5 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-sm font-semibold rounded-lg hover:from-pink-600 hover:to-purple-700 transition-all duration-200 shadow-lg shadow-pink-500/30 hover:shadow-pink-500/50 flex items-center gap-2">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-[#1a1b2e] border border-purple-500/20 rounded-2xl p-6 backdrop-blur-xl">
              <div className="flex items-center justify-between mb-2">
                <lucide_react_1.Key className="w-8 h-8 text-purple-400"/>
            ...
            <div className="bg-[#1a1b2e] border border-purple-500/20 rounded-2xl p-6 backdrop-blur-xl">
              <div className="flex items-center justify-between mb-2">
                <lucide_react_1.TrendingUp className="w-8 h-8 text-green-400"/>
          ...
          {loading ? (<div className="flex items-center justify-center py-20">
              <lucide_react_1.RefreshCw className="w-8 h-8 animate-spin text-purple-400"/>
              <span className="ml-3 text-gray-400">Chargement des clés API...</span>
            </div>) : apiKeys.length === 0 ? (<div className="text-center py-20">
              <lucide_react_1.Key className="w-16 h-16 mx-auto text-gray-500 mb-4"/>
              ...
              <button onClick={() => setShowCreateModal(true)} className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white font-semibold rounded-lg hover:from-pink-600 hover:to-purple-700 transition-all duration-200 shadow-lg shadow-pink-500/30 hover:shadow-pink-500/50 inline-flex items-center gap-2">
                <lucide_react_1.Plus className="w-5 h-5"/>
                Créer une clé API
              </button>
            </div>) : (<div className="space-y-4">
              {apiKeys.map((apiKey) => (<div key={apiKey.id} className="bg-[#1a1b2e] border border-purple-500/20 rounded-2xl p-6 backdrop-blur-xl">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-400">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(apiKey.keyPrefix)} className="text-purple-400 hover:text-purple-300 transition-colors">
                          <lucide_react_1.Copy className="w-4 h-4"/>
                        </button>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 text-xs rounded-full ${apiKey.keyPrefix.includes('test')
                    ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'
                    : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}>
                        {apiKey.keyPrefix.includes('test') ? 'Test' : 'Production'}
                      </span>
                      <button onClick={() => handleRevokeKey(apiKey.id)} disabled={revokingKey === apiKey.id} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-all disabled:opacity-50">
                        {revokingKey === apiKey.id ? (<lucide_react_1.RefreshCw className="w-4 h-4 animate-spin"/>) : (<lucide_react_1.Trash2 className="w-4 h-4"/>)}
                      </button>
                    </div>
                  </div>

                  {/* Usage Stats */}
                  {apiKey.usage && (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div className="bg-[#0f1020] rounded-lg p-3">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-gray-400">Aujourd'hui</span>
                          <lucide_react_1.TrendingUp className="w-3 h-3 text-green-400"/>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="w-full bg-gray-700 rounded-full h-2 mt-2">
                          <div className={`h-2 rounded-full transition-all ${(apiKey.usage.requestsToday / apiKey.quotaRequestsPerDay) < 0.5
                        ? 'bg-green-500'
                        : (apiKey.usage.requestsToday / apiKey.quotaRequestsPerDay) < 0.8
                            ? 'bg-yellow-500'
                            : 'bg-red-500'}`} style={{ width: `${Math.min((apiKey.usage.requestsToday / apiKey.quotaRequestsPerDay) * 100, 100)}%` }}/>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#0f1020] rounded-lg p-3">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <lucide_react_1.Clock className="w-3 h-3 text-blue-400"/>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="w-full bg-gray-700 rounded-full h-2 mt-2">
                          <div className={`h-2 rounded-full transition-all ${(apiKey.usage.requestsThisHour / apiKey.quotaRequestsPerMinute) < 0.5
                        ? 'bg-green-500'
                        : (apiKey.usage.requestsThisHour / apiKey.quotaRequestsPerMinute) < 0.8
                            ? 'bg-yellow-500'
                            : 'bg-red-500'}`} style={{ width: `${Math.min((apiKey.usage.requestsThisHour / apiKey.quotaRequestsPerMinute) * 100, 100)}%` }}/>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#0f1020] rounded-lg p-3">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <lucide_react_1.Check className="w-3 h-3 text-green-400"/>
                        </div>
                        <div className="flex items-center gap-2">
      ...
      {/* Create Key Modal */}
      {showCreateModal && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#1a1b2e] border border-purple-500/20 rounded-2xl p-8 max-w-md w-full mx-4 backdrop-blur-xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} placeholder="Ma clé API" className="w-full px-4 py-3 bg-[#0f1020] border border-purple-500/30 rounded-lg text-white placeholder-gray-500 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 outline-none transition" autoFocus/>
              </div>
              
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <lucide_react_1.AlertTriangle className="w-5 h-5 text-blue-400 mt-0.5"/>
                  <div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreateKey} disabled={!newKeyName.trim() || creatingKey} className="flex-1 px-4 py-3 bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-200 shadow-lg shadow-pink-500/30 hover:shadow-pink-500/50">
                {creatingKey ? (<>
      ...
      {newApiKey && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#1a1b2e] border border-purple-500/20 rounded-2xl p-8 max-w-2xl w-full mx-4 backdrop-blur-xl">
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <lucide_react_1.Check className="w-8 h-8 text-green-400"/>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={newKeyName} readOnly className="w-full px-4 py-3 bg-[#0f1020] border border-purple-500/30 rounded-lg text-gray-400"/>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={copiedKey === newApiKey.key ? "text" : "password"} value={newApiKey.key} readOnly className="w-full px-4 py-3 pr-12 bg-[#0f1020] border border-green-500/30 rounded-lg text-white font-mono text-sm"/>
                  <button onClick={() => handleCopyKey(newApiKey.key)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-green-400 hover:text-green-300 transition-colors">
                    {copiedKey === newApiKey.key ? (<lucide_react_1.Check className="w-5 h-5"/>) : (<lucide_react_1.Copy className="w-5 h-5"/>)}
                  </button>
                </div>
                {copiedKey === newApiKey.key && (<p className="text-sm text-green-400 mt-2">
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mt-6">
              <div className="flex items-start gap-3">
                <lucide_react_1.AlertTriangle className="w-5 h-5 text-red-400 mt-0.5"/>
                <div>
