...
exports.default = CreateChatbotPage;
...
function CreateChatbotPage() {
    ...
    const [createdChatbot, setCreatedChatbot] = (0, react_1.useState)(null);
    // Form state
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-black rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black ${errors.name ? 'border-red-300' : 'border-slate-300'}`} placeholder="Mon Assistant Support"/>
              ...
              <div>
                ...
                <select value={formData.model} onChange={(e) => handleInputChange('model', e.target.value)} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black">
            ...
            <div className="mt-6">
              ...
              <textarea value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} rows={3} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black ${errors.description ? 'border-red-300' : 'border-slate-300'}`} placeholder="Décrivez brièvement ce que fait votre chatbot..."/>
          ...
          <div className="bg-black rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <textarea value={formData.systemPrompt} onChange={(e) => handleInputChange('systemPrompt', e.target.value)} rows={6} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black ${errors.systemPrompt ? 'border-red-300' : 'border-slate-300'}`} placeholder="Décrivez le comportement et les connaissances de votre chatbot..."/>
          ...
          <div className="flex justify-end">
            <button type="submit" disabled={isLoading} className="px-8 py-4 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg font-semibold hover:from-blue-600 hover:to-purple-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
              {isLoading ? (<>
      ...
      {/* Success Modal */}
      {showSuccessModal && createdChatbot && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => copyToClipboard(createdChatbot.publicUrl)} className="text-blue-600 hover:text-blue-800">
                ...
                <div className="flex justify-center">
                  {createdChatbot.qrCode.startsWith('data:image/svg') ? (<div dangerouslySetInnerHTML={{
                    __html: atob(createdChatbot.qrCode.split(',')[1])
                }} className="w-32 h-32 border rounded-lg p-2 bg-white"/>) : (<img src={createdChatbot.qrCode} alt="QR Code" className="w-32 h-32 border rounded-lg"/>)}
                </div>

                <div className="flex gap-3">
                  <button onClick={downloadQRCode} className="flex-1 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
                    <lucide_react_1.Download className="w-4 h-4"/>
                    QR Code
                  </button>
                  <button onClick={() => router.push(`/chat/${createdChatbot.chatbotId}`)} className="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
                    Tester
                  </button>
                </div>
