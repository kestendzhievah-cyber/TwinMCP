...
import { Search, Package, Zap, Settings, CheckCircle, Download, Star, TrendingUp, Lock, Key, Database, Mail, Calendar, Github, Slack, Copy, Eye, EyeOff } from 'lucide-react';
...
export default function MarketplacePage() {
  ...
  const handleInstall = (tool: MCPTool) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2 flex items-center">
            <Package className="w-10 h-10 mr-3 text-purple-400" />
            Marketplace MCP
          </h1>
        ...
        <div className="mb-8 space-y-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Rechercher un outil MCP..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-slate-800/50 border border-purple-500/20 rounded-xl text-black placeholder-gray-500 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none"
            />
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTools.map((tool) => (
            <div
              key={tool.id}
              className="p-6 bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl hover:border-purple-500/50 transition group"
            >
              ...
              <button
                ...
                className="w-full py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-600 transition"
              >
            ...
            <div className="bg-black rounded-2xl border border-purple-500/20 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  {selectedTool.apiFields.map((field: ApiField) => (
                    <div key={field.name}>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type={showApiKey[field.name] ? 'text' : field.type}
                          value={apiConfig[field.name] || ''}
                          onChange={(e) => setApiConfig({ ...apiConfig, [field.name]: e.target.value })}
                          placeholder={`Votre ${field.label.toLowerCase()}`}
                          className="w-full pl-11 pr-11 py-3 bg-slate-700/50 border border-slate-600 text-black rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none"
                        />
                        {field.type === 'password' && (
                          <button
                            onClick={() => setShowApiKey({ ...showApiKey, [field.name]: !showApiKey[field.name] })}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white"
                          >
                            {showApiKey[field.name] ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                          </button>
                        )}
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedTool.capabilities.map((cap: string, idx: number) => (
              ...
              <div className="p-6 border-t border-purple-500/20 flex space-x-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-600 transition"
                >
