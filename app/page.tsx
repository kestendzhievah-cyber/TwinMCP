            ...
            {plans.map((plan, index) => (
              <div 
                ...
                className={`relative p-8 rounded-2xl border transition-all duration-300 ${
                  plan.popular 
                    ? 'bg-gradient-to-br from-purple-900/50 to-pink-900/50 border-purple-500 shadow-2xl shadow-purple-500/30 scale-105 animate-pulse-glow' 
                    : 'bg-slate-800/50 border-slate-700 hover-lift hover-glow'
                } ${pricingSection.isInView ? `animate-fade-in-scale delay-${(index + 1) * 100}` : 'opacity-0'}`}
              >
                {plan.popular && (
