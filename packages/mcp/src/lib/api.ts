...
import { TWINMCP_API_BASE_URL } from "./constants.js";
import { readFileSync } from "fs";
import tls from "tls";

/**
 * Parses error response from the TwinMCP API
...
export function loadCustomCACerts(customCACertsPath = CUSTOM_CA_CERTS): string[] | undefined {
  ...
  try {
    ...
    console.error(
      `[TwinMCP] Failed to load custom CA certificates from ${customCACertsPath}:`,
      error
    );
    return undefined;
  }
...
if (PROXY_URL && !PROXY_URL.startsWith("$") && /^(http|https):\/\//i.test(PROXY_URL)) {
  try {
    ...
    console.error(
      `[TwinMCP] Failed to configure proxy agent for provided proxy URL: ${PROXY_URL}:`,
      error
    );
  }
  ...
  if (ca) {
    try {
      ...
      console.error(`[TwinMCP] Failed to configure custom CA certificates:`, error);
    }
  }
}
...
export async function searchLibraries(
  ...
  try {
    const url = new URL(`${TWINMCP_API_BASE_URL}/v2/libs/search`);
...
export async function fetchLibraryContext(
  ...
  try {
    const url = new URL(`${TWINMCP_API_BASE_URL}/v2/context`);
    ...
    if (!text) {
      return {
        data: "Documentation not found or not finalized for this library. This might have happened because you used an invalid TwinMCP-compatible library ID. To get a valid TwinMCP-compatible library ID, use the 'resolve-library-id' with the package name you wish to retrieve documentation for.",
      };
    }
    return { data: text };
  } catch (error) {
