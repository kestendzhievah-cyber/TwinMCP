...
/**
 * Generate headers for TwinMCP API requests.
...
export function generateHeaders(context: ClientContext): Record<string, string> {
  const headers: Record<string, string> = {
    "X-TwinMCP-Source": "mcp-server",
    "X-TwinMCP-Server-Version": SERVER_VERSION,
  };
  ...
  if (context.clientInfo?.ide) {
    headers["X-TwinMCP-Client-IDE"] = context.clientInfo.ide;
  }
  if (context.clientInfo?.version) {
    headers["X-TwinMCP-Client-Version"] = context.clientInfo.version;
  }
  if (context.transport) {
    headers["X-TwinMCP-Transport"] = context.transport;
  }
