...
const program = new Command()
  ...
  .option("--api-key <key>", "API key for authentication (or set TWINMCP_API_KEY env var)")
...
const server = new McpServer(
  {
    ...
    websiteUrl: "https://twinmcp.com",
    description:
      "TwinMCP provides up-to-date documentation and code examples for libraries and frameworks.",
    icons: [
      {
        src: "https://twinmcp.com/twinmcp-icon-green.png",
        mimeType: "image/png",
      },
    ],
  },
...
server.registerTool(
  "resolve-library-id",
  {
    title: "Resolve TwinMCP Library ID",
    description: `Resolves a package/product name to a TwinMCP-compatible library ID and returns matching libraries.

You MUST call this function before 'Query Documentation' tool to obtain a valid TwinMCP-compatible library ID UNLESS the user explicitly provides a library ID in the format '/org/project' or '/org/project/version' in their query.

Each result includes:
- Library ID: TwinMCP-compatible identifier (format: /org/project)
...
IMPORTANT: Do not call this tool more than 3 times per question. If you cannot find what you need after 3 calls, use the best result you have.`,
    inputSchema: {
      query: z
        .string()
        .describe(
          "The question or task you need help with. This is used to rank library results by relevance to what the user is trying to accomplish. The query is sent to the TwinMCP API for processing. Do not include any sensitive or confidential information such as API keys, passwords, credentials, personal data, or proprietary code in your query."
        ),
      libraryName: z
        .string()
        .describe(
...
server.registerTool(
  "query-docs",
  {
    ...
    description: `Retrieves and queries up-to-date documentation and code examples from TwinMCP for any programming library or framework.

You must call 'Resolve TwinMCP Library ID' tool first to obtain the exact TwinMCP-compatible library ID required to use this tool, UNLESS the user explicitly provides a library ID in the format '/org/project' or '/org/project/version' in their query.
...
IMPORTANT: Do not call this tool more than 3 times per question. If you cannot find what you need after 3 calls, use the best information you have.`,
    inputSchema: {
      libraryId: z
        .string()
        .describe(
          "Exact TwinMCP-compatible library ID (e.g., '/mongodb/docs', '/vercel/next.js', '/supabase/supabase', '/vercel/next.js/v14.3.0-canary.87') retrieved from 'resolve-library-id' or directly from user query in the format '/org/project' or '/org/project/version'."
        ),
      query: z
        .string()
        .describe(
          "The question or task you need help with. Be specific and include relevant details. Good: 'How to set up authentication with JWT in Express.js' or 'React useEffect cleanup function examples'. Bad: 'auth' or 'hooks'. The query is sent to the TwinMCP API for processing. Do not include any sensitive or confidential information such as API keys, passwords, credentials, personal data, or proprietary code in your query."
        ),
    },
    annotations: {
      readOnlyHint: true,
  ...
  if (transportType === "http") {
    ...
    app.use((req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      res.setHeader(
        ...
        "Content-Type, MCP-Session-Id, MCP-Protocol-Version, X-TwinMCP-API-Key, TwinMCP-API-Key, X-API-Key, Authorization"
      );
    ...
    const extractApiKey = (req: express.Request): string | undefined => {
      return (
        ...
        extractHeaderValue(req.headers["twinmcp-api-key"]) ||
        extractHeaderValue(req.headers["x-api-key"]) ||
        extractHeaderValue(req.headers["twinmcp_api_key"]) ||
    ...
    const startServer = (port: number, maxAttempts = 10) => {
      ...
      httpServer.once("listening", () => {
        console.error(
          `TwinMCP Documentation MCP Server v${SERVER_VERSION} running on HTTP at http://localhost:${port}/mcp`
        );
      });
    };
    ...
    stdioApiKey = cliOptions.apiKey || process.env.TWINMCP_API_KEY;
    const transport = new StdioServerTransport();

    await server.connect(transport);

    console.error(`TwinMCP Documentation MCP Server v${SERVER_VERSION} running on stdio`);
  }
}
