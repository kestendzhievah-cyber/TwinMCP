...
class TextUtils {
    ...
    static looksLikeLibraryName(token) {
    ...
    static calculateTokenConfidence(token) {
        let confidence = 0.5;
        ...
        for (let i = 0; i <= str2.length; i++) {
            for (let j = 0; j <= str1.length; j++) {
                if (i === 0) {
                    ...
                    matrix[i][j] = Math.min((prevRow[j] ?? 0) + 1, // deletion
