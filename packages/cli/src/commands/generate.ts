...
async function generateCommand(options: GenerateOptions): Promise<void> {
  ...
  if (quota.tier !== "unlimited" && quota.remaining < 1) {
    ...
    if (quota.tier === "free") {
      ...
      console.log(`  Visit ${pc.green("https://twinmcp.com/dashboard")} to upgrade.`);
    }
    return;
  }
  ...
  if (searchResult.searchFilterApplied) {
    log.warn(
      "Your results only include libraries matching your teamspace's library filters. To adjust quality thresholds or blocked libraries, update your filters at https://twinmcp.com/dashboard?tab=policies"
    );
    log.blank();
  }
  ...
  let initialStatus = "Reading selected TwinMCP sources to generate the skill...";
      ...
      if (genSpinner) {
        if (event.message.startsWith("Generating skill content...") && !isGeneratingContent) {
          ...
          if (queryLog.length > 0) {
            genSpinner.succeed(pc.green(`Read TwinMCP sources`));
          } else {
  ...
  while (true) {
    ...
    initialStatus = feedback
      ...
      : "Reading selected TwinMCP sources to generate the skill...";
    ...
    const openInEditor = async () => {
      const previewDir = join(homedir(), ".twinmcp", "previews");
