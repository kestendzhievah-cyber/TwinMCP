...
function formatLibraryResult(lib: LibrarySearchResult, index: number): string {
  ...
  lines.push(`   ${pc.cyan(`TwinMCP-compatible library ID: ${lib.id}`)}`);
...
export function registerDocsCommands(program: Command): void {
  program
    ...
    .description("Resolve a library name to a TwinMCP library ID")
  ...
  program
    .command("docs")
    .argument("<libraryId>", "TwinMCP library ID (e.g., /facebook/react)")
