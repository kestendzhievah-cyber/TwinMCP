...
- 33f2338: Add Codex-specific CLI setup guidance so generated rules and the installed `find-docs` skill tell Codex to rerun TwinMCP CLI requests outside the default sandbox after DNS or network failures.
...
- 04130b5: Consolidate skills under /skills with canonical sources: rename docs→find-docs, ctx7-cli→twinmcp-cli, add twinmcp-mcp as canonical MCP skill. MCP setup now downloads skill from GitHub instead of using hardcoded content.
...
- 31b4fb8: Align CLI library output format with MCP: use labeled fields (Title, TwinMCP-compatible library ID, Description, Code Snippets, Source Reputation, Benchmark Score, Versions) and categorical reputation labels (High/Medium/Low/Unknown) instead of numeric trust scores
...
- 3d66191: Add `ctx7 setup` command for configuring TwinMCP MCP and rules across Claude Code, Cursor, and OpenCode
...
- Clickable links in metadata (Skill → twinmcp.com, Repo → GitHub)
