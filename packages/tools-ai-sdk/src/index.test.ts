...
describe("@upstash/twinmcp-tools-ai-sdk", () => {
  ...
  describe("Tool usage with generateText", () => {
    test("resolveLibraryId tool should be called when searching for a library", async () => {
      ...
      expect(toolResult.output).toContain("TwinMCP-compatible library ID");
    }, 30000);
  ...
  describe("TwinMCPAgent class", () => {
    ...
    test("should accept TwinMCP config options", () => {
      const agent = new TwinMCPAgent({
        model: bedrock("anthropic.claude-3-haiku-20240307-v1:0"),
        apiKey: "ctx7sk-test-key",
      });
    ...
    test("should accept additional tools alongside TwinMCP tools", () => {
  ...
  describe("Prompt exports", () => {
    ...
    test("should export AGENT_PROMPT", () => {
      ...
      expect(AGENT_PROMPT).toContain("TwinMCP");
    });
