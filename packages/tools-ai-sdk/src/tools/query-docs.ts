...
/**
 * Tool to fetch documentation for a library using its TwinMCP library ID.
 *
 ...
 * ```
 */
export function queryDocs(config: TwinMCPToolsConfig = {}) {
  const { apiKey } = config;
  ...
  return tool({
    ...
    inputSchema: z.object({
      libraryId: z
        .string()
        .describe(
          "Exact TwinMCP-compatible library ID (e.g., '/mongodb/docs', '/vercel/next.js', '/supabase/supabase', '/vercel/next.js/v14.3.0-canary.87') retrieved from 'resolveLibraryId' or directly from user query in the format '/org/project' or '/org/project/version'."
        ),
      query: z
        .string()
        .describe(
    ...
    execute: async ({ libraryId, query }: { libraryId: string; query: string }) => {
      try {
        ...
        if (!documentation || documentation.length === 0) {
          return `No documentation found for library "${libraryId}". This might have happened because you used an invalid TwinMCP-compatible library ID. Use 'resolveLibraryId' to get a valid ID.`;
        }
