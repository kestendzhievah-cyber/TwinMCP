...
const DEFAULT_BASE_URL = "https://twinmcp.com/api";
const API_KEY_PREFIX = "ctx7sk";

export type * from "@commands/types";
export * from "@error";

export class TwinMCP {
  private httpClient: HttpClient;

  constructor(config: TwinMCPConfig = {}) {
    const apiKey = config.apiKey || process.env.TWINMCP_API_KEY;

    if (!apiKey) {
      throw new TwinMCPError(
        "API key is required. Pass it in the config or set TWINMCP_API_KEY environment variable."
      );
    }
   ...
   * @param libraryId TwinMCP library ID (e.g., "/facebook/react")
   * @param options Response format options
