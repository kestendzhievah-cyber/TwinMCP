...
export type TwinMCPResponse<TResult> = {
  result?: TResult;
  headers?: TxtResponseHeaders;
};

export type Requester = {
  request: <TResult = unknown>(req: TwinMCPRequest) => Promise<TwinMCPResponse<TResult>>;
};
...
export class HttpClient implements Requester {
  ...
  public async request<TResult>(req: TwinMCPRequest): Promise<TwinMCPResponse<TResult>> {
    ...
    if (!res.ok) {
      ...
      throw new TwinMCPError(errorBody.error || errorBody.message || res.statusText);
    }
  ...
  private extractTxtResponseHeaders(headers: Headers): TxtResponseHeaders | undefined {
    const page = headers.get("x-twinmcp-page");
    const limit = headers.get("x-twinmcp-limit");
    const totalPages = headers.get("x-twinmcp-total-pages");
    const hasNext = headers.get("x-twinmcp-has-next");
    const hasPrev = headers.get("x-twinmcp-has-prev");
    const totalTokens = headers.get("x-twinmcp-total-tokens");
